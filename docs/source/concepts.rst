...
To learn a classifier we use a training set that provides `n_samples` of sampled objects with their labels. To perform multi-label classification we need a training set. 

In the next section you will learn about the data format scikit-multilearn expects.